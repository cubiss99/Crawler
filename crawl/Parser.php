...
abstract class Parser
{
    ...
    ) 
    {
        ...
        if ($result) {
            ...
            echo '<script>alert("thất bại ' . $sql . '!!!");</script>;';
        }
    }
     ...
     function articleParser($url)
    {
        ...
        $description = $finder
            ->query("//div[@class='dt-news__sapo']/h2")
            ->item(0); // xpath lay DOM doan van ngan
        $date = $finder->query("//span[@class='dt-news__time']")->item(0); // xpath lay DOM ngay thang
        $category = $finder->query(
            "//ul[@class='dt-breadcrumb']/li[position() > 1]"
        ); // xpath lay DOM the loai
